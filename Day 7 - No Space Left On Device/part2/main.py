...
class Folder:
    ...
    def get_sum(self):
        total_sum = 0
        if not self.file and self.size!=0:
            total_sum += self.size
        for i in self.after:
            total_sum += i.get_sum()
        return total_sum
    
    def get_near(self, num, current=None):
        if current==None:
            current=self
        else:
            if self.size>=num and self.size<current.size:
                current=self        
        for i in self.after:
            current = i.get_near(num, current)        
        return current
...
free_memory = 70000000 - computer.size
need_free = 30000000 - free_memory
print(computer)

delete = computer.get_near(need_free)
print(delete.size, delete.name)